    ...
    if (persons.find(person => person.name === newName)===undefined) {
      ...
      else
      {
        if (window.confirm(`${newName} is already added to phonebook, replace the old number with a new one?`)) {          
          UpdatePerson(persons.find(person=>person.name===newName).id,newNumber)
        }
        
      }
}
const UpdatePerson = (id,number) => {
      SetPersonUpdate(persons.find(person => person.id === id))
      const chengedPerson={...personUpdate,name:newName,number:number}
      PersonsServices
      .update(id,chengedPerson)
      .then(response=>{
        setCssClass('success')
